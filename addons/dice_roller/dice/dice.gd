...
func show_face(value):
	...
	dehighlight()
	rolling = true
	const pre_spin_time := 0.38
	const settle_time := 0.34
	var rotated := face_up_transform(value)
	var spin_rotation := rotation + Vector3(
		TAU * randf_range(1.35, 1.85),
		TAU * randf_range(1.10, 1.55),
		TAU * randf_range(1.55, 2.10)
	)
	var tween := create_tween()
	tween.tween_property(self, "rotation", spin_rotation, pre_spin_time).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	tween.tween_property(self, "transform", rotated, settle_time).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
