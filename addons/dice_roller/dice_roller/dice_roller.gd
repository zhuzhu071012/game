...
func set_default_dice_set():
	var new_set: Array[Resource] = []
	for name in default_set:
		var dice = DICE_DEF_SCRIPT.new()
		dice.name = name
		dice.color = default_set[name].color
		dice.shape = DICE_SHAPE_SCRIPT.new("D6")
		new_set.append(dice)
	dice_set = new_set
...
func ensure_valid_and_unique_dice_names():
	...
	for dice in dice_set:
		var name_prefix: String = str(dice.name)
...
func add_dice_escene(dice):
	var packed_scene: PackedScene
	if not dice.shape:
		push_warning("Shapeless dice def ", dice.name)
		packed_scene = DICE_SHAPE_SCRIPT.new("D6").scene()
	else:
...
func quick_roll():
	...
	for dice in dices:
		var dice_values: Array = dice.sides.keys()
